#last edit, 21/10/2019, the previous string erodil detection method. accoutning for blobs
...
# string expanding - finals step, if two strings are separated by a blob, this will "join them"
v_string_fin_open = 9 # default is 9, the learger this value the larger the gap that can be filled between two neighbouring strings
#blob finding
v_blob_find = 3 # default is 3, larger is reduced sensetivity to detecting blobs
###-----end
...
def stringSegmentation(convolve_image, str_image, blob_image, image_name, file_path):
    ...
    score_overall_erodil = skimage.morphology.dilation(score_overall_erodil, np.ones((3, v_string_fin_open))) 
    score_overall_erodil = skimage.morphology.erosion(score_overall_erodil, np.ones((3, v_string_fin_open))) 
    ...
    imsave(file_path+"/04_python_images/"+image_name.split('.',1)[0]+'.png', strings_out_erodil)
...
for image in range(0, number_of_images):
    ...
    im_blob = skimage.morphology.erosion(im_blob, np.ones((v_blob_find,v_blob_find)))
