#1_2 altering variables to match fp
#Author: Oscar Charles
#User: Francesca Patella, ISMB UCL Cutlerlab
#Purpose: Takes an image of VWF immu-stained "strings" from a flow cell, identifies strings and returns a dat csv & image file per in-image.
#v1.2 - Jan 2018 - Script works as desired, although a little rough.
#v1.3 - April 2019 - Update to remove false positive border strings, implement automated imagej convolve step.

#os.chdir('C:\\OneDrive\\UCL\\1-Cutler\\6-Patella_Strings\\1825_forOscar\\1825_strings_auto_test')
script_name = "2"
import numpy as np
import pandas as pd
import skimage
...
def stringSegmentation(String_image, image_name):   
    ...
    imsave(file_path+"/04_python_images/"+ script_name +"_"+image_name.split('.',1)[0]+'.png', String_seg)
...
for image in range(0, number_of_images):
    ...
    String_output_image, String_output_data = stringSegmentation(String_image, image_name)
